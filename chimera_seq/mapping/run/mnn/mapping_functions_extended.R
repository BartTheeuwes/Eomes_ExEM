...
getMappingScore <- function(mapping){
    ...
    celltypes_extended_accrossK <- matrix(unlist(mapping$celltypes_extended.mapped), # ADDED
                                 nrow=length(mapping$celltypes_extended.mapped[[1]]),
                                 ncol=length(mapping$celltypes_extended.mapped))
    ...
    out$celltype_extended.score <- NULL # ADDED
    for (i in 1:nrow(celltypes_extended_accrossK)){
        p <- max(table(celltypes_extended_accrossK[i,]))
        index <- which(table(celltypes_extended_accrossK[i,]) == p)
        p <- p/length(mapping$celltypes_extended.mapped)
        out$celltype_extended.score <- c(out$celltype_extended.score,p)
    }
...
get_meta <- function(pca_atlas, meta_atlas, pca_query, meta_query, k = 10){
  ...
  #get closest k matching cells
    ...
    celltypes_extended <- t(apply(k.mapped, 1, function(x) meta_atlas$celltype_extended[match(x, meta_atlas$cell)]))
    ...
    celltype_extended.mapped <- apply(celltypes_extended, 1, function(x) getmode(x, 1:length(x)))
  ...
  out <- lapply(1:length(celltype.mapped), function(x){
    list(cells.mapped     = k.mapped[x,],
         ...
         celltype_extended.mapped  = celltype_extended.mapped[x],
         stage.mapped     = stage.mapped[x],
         celltypes.mapped = celltypes[x,],
         celltypes_extended.mapped = celltypes_extended[x,],
...
mapWrap <- function(sce_atlas, meta_atlas, sce_query, meta_query, order = NULL, k = 30, npcs = 50, genes = NULL, cosineNorm = FALSE){
  ...
  for (i in seq(from = 1, to = k)) {
    ...
    out$celltypes_extended.mapped[[i]]  <- sapply(mapping, function(x) x$celltypes_extended.mapped[i])
  ...
  cte <- sapply(mapping, function(x) x$celltype_extended.mapped); is.na(cte) <- lengths(cte) == 0
  ...
  out$mapping <- data.frame(
      ...
      celltype_extended.mapped = unlist(cte),
