...
dir.create(args$outputdir, showWarnings = F, recursive = T)
...
to.plot <- metadata %>% 
    ...
    melt(id.vars=c("sample","cell","day"), measure.vars=c("nCount_RNA", "nFeature_RNA","mitochondrial_percent_RNA","ribosomal_percent_RNA"))
...
## Box plot 
p <- ggplot(to.plot, aes_string(x="sample", y="value", fill="day")) +
    geom_violin() +
    ...
    scale_fill_manual(values=opts$days.colors) +
    theme_classic() +
    theme(
...
to.plot <- metadata %>%
    .[,mean(pass_rnaQC,na.rm=T),by=c("sample","day")]

p <- ggbarplot(to.plot, x="sample", y="V1", fill="day") +
    scale_fill_manual(values=opts$days.colors) +
