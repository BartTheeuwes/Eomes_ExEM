...
sample_metadata <- fread(args$query_metadata) %>%
    ...
    .[, c("closest.cell_mnn","celltype.mapped_mnn", "celltype_extended.mapped_mnn", "genotype")] %>%
    ...
    .[, mapped:="Chimaera"] %>%
    .[, c('cell', 'V1', 'V2', 'celltype', 'celltype_extended', 'mapped', 'genotype')]

to.plot = rbind(umap.dt, copy(sample_metadata)[,genotype:=NULL])
...
# Plot celltypes
to.plot = rbind(copy(umap.dt)[,genotype:='KO'], copy(umap.dt)[,genotype:='WT'], sample_metadata)
