...
args$max_tss_enrichment <- 40
args$min_number_fragments <- 3500
args$max_number_fragments <- 300000
...
# rownames(ArchRProject) %in% sample_metadata$cell
# sample_metadata[!cell %in% rownames(ArchRProject)]
ArchRProject
...
pdf(sprintf("%s/qc_TSSenrichment.pdf",args$outdir), width=9, height=7)
print(p)
dev.off()
...
}) %>% rbindlist %>% setnames("group","sample")
...
pdf(sprintf("%s/qc_FragmentSizeDistribution.pdf",args$outdir), width=9, height=7)
print(p)
dev.off()
...
pdf(sprintf("%s/qc_metrics_histogram.pdf",args$outdir), width=10, height=5)
print(p)
dev.off()
...
pdf(sprintf("%s/qc_metrics_barplot.pdf",args$outdir), width=9, height=6)
print(p)
dev.off()
